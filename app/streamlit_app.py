...
def main():
    ...
    analysis_type = st.sidebar.selectbox(
        "Select Analysis",
        ["Overview", "Enhanced Summary", "Resistance Heatmap", "Cluster Analysis", 
         "Cluster Validation", "Species Classifier", "Co-Resistance Network", "PCA Analysis", 
         "Regional Distribution", "Local Site Distribution", "Source Distribution",
         "Species Distribution", "Model Evaluation", "Integration & Synthesis",
    ...
    if analysis_type == "Overview":
    ...
    elif analysis_type == "Enhanced Summary":
        ...
        st.header("✅ Cluster Validation (k=5 Justification)")
        
        st.markdown("""
        <div class="info-box">
            <strong>ℹ️ Validation Method:</strong> k=5 was selected using a <strong>combined approach</strong>:
        ...
        if os.path.exists(metrics_path):
            ...
            with col2:
                # Display key metrics for k=5
                k5_row = metrics_df[metrics_df['k'] == 5].iloc[0] if 5 in metrics_df['k'].values else None
                if k5_row is not None:
                    st.subheader("k=5 Selection Rationale")
                    
                    # Metrics display
                    st.metric("Silhouette Score", f"{k5_row['silhouette_score']:.3f}")
                    st.metric("WCSS", f"{k5_row['wcss']:.0f}")
                    ...
                    k5_sil = k5_row['silhouette_score']
                    
                    # Calculate elbow point approximation
                    wcss_diff = metrics_df['wcss'].diff().abs()
                    wcss_diff_change = wcss_diff.diff()
                    
                    st.markdown("---")
                    st.markdown("**Why k=5 instead of higher k?**")
                    
                    # Detailed justification
                    st.markdown(f"""
                    1. **Elbow Point:** WCSS reduction slows significantly around k=4-5 (diminishing returns)
                    
                    2. **Sample Size:** At k=5, smallest cluster has ~23 isolates (adequate for statistics). Higher k values create clusters with <20 isolates, reducing statistical power.
                    
                    3. **Silhouette Trade-off:** While k={int(best_k)} has highest silhouette ({best_sil:.3f}), k=5 ({k5_sil:.3f}) still indicates "strong clustering" (>0.4). The difference ({best_sil - k5_sil:.3f}) is marginal.
                    
                    4. **Interpretability:** 5 clusters align with known resistance phenotypes in the literature.
                    """)
            ...
            with col2:
                if os.path.exists(silhouette_plot):
                    st.image(silhouette_plot, caption="Silhouette Detail for k=5")
            ...
            with st.expander("📚 Methodology Details"):
                ...
                **Combined Selection Algorithm:**
                ...
                The k=5 selection follows a principled approach:
                
                1. **Elbow Detection:** Using the kneedle algorithm, the elbow point (diminishing WCSS returns) is identified around k=4-5.
                
                2. **Silhouette Validation:** Any k with silhouette ≥0.40 is considered "strong clustering." k=5 exceeds this threshold.
                
                3. **Stability Check:** Cluster sizes are verified to ensure minimum sample sizes (≥20) for reliable statistics.
                
                4. **Literature Alignment:** 5 clusters is consistent with AMR phenotype studies that typically identify 3-7 major resistance patterns.
    ...
    elif analysis_type == "Species Classifier":
        st.header("🧬 Species Classifier Results")
        ...
        st.subheader("📈 Co-Resistance Model Performance Summary")
        ...
        if os.path.exists(pred_path):
            ...
            with col1:
                ...
                colors = ['#4CAF50' if x > 0.9 else '#FF9800' if x > 0.7 else '#F44336' 
            ...
            with col2:
                ...
                bars1 = ax.bar(x - width*1.5, pred_df['AUC'], width, label='AUC', color='#4CAF50', edgecolor='black')
                bars2 = ax.bar(x - width/2, pred_df['Accuracy'], width, label='Accuracy', color='#2196F3', edgecolor='black')
                bars3 = ax.bar(x + width/2, pred_df['Precision'], width, label='Precision', color='#FF9800', edgecolor='black')
                bars4 = ax.bar(x + width*1.5, pred_df['Recall'], width, label='Recall', color='#9C27B0', edgecolor='black')
            ...
            **🎯 Co-Resistance Model Summary:**
            ...
            st.warning("Prediction results not found. Run `python scripts/coresistance_analysis.py` to generate.")
            ...
            try:
                ...
                with col1:
                    if comparison.get('cluster_mdr_alignment'):
                        ...
                        st.dataframe(mdr_df, width='stretch')
                ...
                with col2:
                    if comparison.get('cluster_purity'):
                        ...
                        st.dataframe(pd.DataFrame(purity_data), width='stretch')
